 ...
 */
exports.getSocials = function(req,res){
...
exports.setDb = function(req,res){
  ...
  connection.connect(function(err){
    ...
    console.log('You are now connected...');
    if (req.body.count <=0)
      return res.send("count must be big than 0");
    var values = [];
    for (var i = 0; i < req.body.count; i++) {
      var aValue = [];
      aValue.push(randomstring.generate(5));
      aValue.push(randomstring.generate(5) + '@' + randomstring.generate(3) + '.com');
      ...
      aValue.push(randomnumber({min: 0, max: 1000, integer: true}));
      aValue.push(randomnumber({min: 0, max: 1000, integer: true}));
      aValue.push(randomstring.generate(10));
      values.push(aValue);
    }
